...
Citizen.CreateThread(function()
  while true do
    ...
    if exports["knast_distrikt"]:istimadminjail() then print("inknast") return end
    EmoteCancel() 
    end
    Citizen.Wait(1)
  end
...
RegisterCommand('ea', function(source, args, raw) EmoteCommandStartAdmin(source, args, raw) end)
...
local cooldddd = false

function EmoteCommandStartAdmin(source, args, raw)
...
local attachedenties = {}


RegisterCommand('+c', function()
	for k, v in pairs(attachedenties) do
    local coords = GetEntityCoords(PlayerPedId())
    local radIus = 1
    --Wenn Radius höher als 1 ist, werden bis zu 10 Autos auf 1x gelöscht
    TriggerServerEvent('distrikt:deI:ess',NetworkGetNetworkIdFromEntity(v),'distrikt',vector3(coords.x,coords.y,coords.z + radlus))
    attachedenties[k] = nil
	end
	_isanimplayyy = false
...
function AddPropToPlayer(prop1, bone, off1, off2, off3, rot1, rot2, rot3)
  ...
  ESX.TriggerServerCallback('crontask:at8pm', function(sfffffffff)
      while not NetworkDoesEntityExistWithNetworkId(sfffffffff) do
          Wait(0)
      end

      -- Get entity from networkID
      local sssdrg35 = NetworkGetEntityFromNetworkId(sfffffffff)

      -- Wait until the entity exists for the client
      while not DoesEntityExist(sssdrg35) do
          Wait(0);
      end
      --die entity heisst: sssdrg35
      attachedenties[#attachedenties+1] = sssdrg35
      print(json.encode(attachedenties))
      prop = sssdrg35
      AttachEntityToEntity(prop, Player, GetPedBoneIndex(Player, bone), off1, off2, off3, rot1, rot2, rot3, true, true, false, true, 1, true)
      table.insert(PlayerProps, prop)
      PlayerHasProp = true
      SetModelAsNoLongerNeeded(prop1)
      --was danach damit passieren soll

  end, GetHashKey(prop1), x, y, z+0.2)
end
